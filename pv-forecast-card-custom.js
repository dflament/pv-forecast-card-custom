...
console.info("📦 clock-pv-forecast-card-custom v1.1.5 loaded");
...
class ClockPvForecastCard extends LitElement {
  ...
  setConfig(config) {
    ...
    this.config = {
	  ...
	  max_value_entity: config.max_value_entity || null,
  ...
  _barWidth(value) {
    ...
    if (this.config.max_value_entity) {
	  const state = this.hass.states[this.config.max_value_entity];
...
if (!customElements.get('clock-pv-forecast-card-custom')) {
  customElements.define('clock-pv-forecast-card-custom', ClockPvForecastCard);
}
